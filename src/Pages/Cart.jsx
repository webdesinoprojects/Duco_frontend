    ...
    >
      ...
      <div style={{ marginTop: '10px' }}>
        <p>
          ...
          {data.charges.pfFlat > 0
            ? ` (includes flat ₹${safeNum(data.charges.pfFlat).toFixed(2)})`
            : ''}
        </p>
...
const Cart = () => {
  ...
  const subtotal = useMemo(() => {
    const result = actualData.reduce((sum, item) => {
      const qty = Object.values(item.quantity || {}).reduce(
        (a, q) => a + safeNum(q),
        0
      );
      const itemTotal = safeNum(item.price) * qty;
      ...
      console.log('🧮 Cart Item Calculation:', {
        ...
        quantity: qty,
        itemTotal: itemTotal,
        quantityBreakdown: item.quantity,
      });
  ...
  const pfTotal = 0; // Temporarily set to 0 for testing
  ...
  const orderPayload = {
    ...
    pfFlat: 0, // Temporarily set to 0 for testing
    gst: gstPercent,
          ...
          >
            ...
            <button
              ...
              onClick={() => {
                ...
                // ✅ Log order payload being sent to payment
                console.log('🔍 ORDER PAYLOAD FROM CART:', {
                  orderPayload,
                  paymentOptions,
                  isB2B,
                  actualData,
